...
class User(db.Model):
    ...
    name = db.Column(db.String, unique=False)  # a username must be unique! Two users cannot have the same username
    email = db.Column(db.String, unique=True)  # email must be unique! Two users cannot have the same email address
    password = db.Column(db.String, unique=False)  # password
